...
public class SanPhamsController : Controller
{
    ...
    public JsonResult ToggleBanChay(string id)
    {
        ...
        // Đảo trạng thái BanChay (0 ↔ 1)
        sp.BanChay = (sp.BanChay == 1) ? 0 : 1;

        try
        {
            db.SaveChanges();
            return Json(new { success = true, status = sp.BanChay });
        }
        catch (Exception ex)
        {
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public ActionResult Create([Bind(Include = "MaLoaiSP,MaHang,TenSP,SoLuong,Link,KhuyenMai,TuKhoa,GiaNhap,GiaDau,SoGiam,MoTa,Status,BanChay")] SanPham sanPham, HttpPostedFileBase HinhAnh)
    {
